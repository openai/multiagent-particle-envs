    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                agent.action.u = np.zeros(self.world.dim_p)
                index = np.random.choice(5, p=action[0])
                # process discrete action
                if index == 1: agent.action.u[0] = -1.0
                if index == 2: agent.action.u[0] = +1.0
                if index == 3: agent.action.u[1] = -1.0
                if index == 4: agent.action.u[1] = +1.0
            else:
        ...
        if not agent.silent:
            ...
            if self.discrete_action_input:
                index = np.random.choice(3, p=action[0])
                agent.action.c = np.zeros(self.world.dim_c)
