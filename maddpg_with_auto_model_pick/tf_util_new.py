...
def load_state(fname, load_best=False, saver=None):
    """Load all the variables to the current session from the location <fname>"""
    if saver is None:
        saver = tf.train.Saver()
    if load_best:
        saver.restore(get_session(), fname[:-1]+"_best"+fname[-1])
    else:
        saver.restore(get_session(), fname)
    return saver


def save_state(fname, save_best=False, saver=None):
    """Save all the variables in the current session to the location <fname>"""
    os.makedirs(os.path.dirname(fname), exist_ok=True)
    if saver is None:
        saver = tf.train.Saver()
    if save_best:
        saver.save(get_session(), fname[:-1]+"_best"+fname[-1])
